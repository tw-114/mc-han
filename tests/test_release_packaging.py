...
def test_package_runtime_and_release_names_use_one_version_source():
    ...
    assert expected == "0.7.0b2"
    assert mc_han.__version__ == expected
    assert get_version() == expected
    assert release_tag() == "v0.7.0-beta.2"
    assert windows_archive_name() == "mc-han-windows-x64-v0.7.0-beta.2.zip"
    assert windows_checksum_name() == (
        "mc-han-windows-x64-v0.7.0-beta.2.sha256"
    )
...
def test_release_notes_and_notices_describe_preview_boundary():
    notes = (
        ROOT / "release-notes" / "v0.7.0-beta.2.md"
